...
>
> This is the worked example belonging to the `spec-creating` skill. It specifies a feature of *Bookshelf*, a
...
the same decision, for the same reasons, as [ADR-2](../../adr/2-schema-change-handling.md).
...
A new flow, `flows/book-rating/book-rating.yaml` — this is the capability's first slice, so the folder does
not exist yet.
