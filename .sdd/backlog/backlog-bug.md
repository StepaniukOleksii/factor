...
4. On the Create Observation screen when there are more elements to display than the device screen can fit, the Add
   Metric button is overlapped by the Create Observation button.
5. It is possible to create multiple Observations with the same name.
6. A preset and a custom range covering the same span bucket their Records differently, so switching from `1Y`
...
7. The numeric chart cannot display more than 5 or 6 numbers. The leading number is silently hidden. 
8. The Create Observation screen marks no field as required until a save is refused. Requiredness is only
