...
* Every chart in the Trends section reserves one gutter of the same width for the labels down its left edge,
  so every card puts a given moment at the same place across its plot and one metric can be read against
  another down a column. That replaces the two widths in use — the Numeric chart's, too narrow for the value
  labels it already draws, and the Enum chart's, wider than any card needs it to be.
...
* No new layout beyond that shared gutter: the chart is otherwise the two-lane case of the Enum chart's
...
Metric has, and the canonical strings they match a series on, are this chart's business — `metricDisplay`
owns only how a value is spoken.

**One label gutter replaces two.** `chartAxis.tsx` gains `LABEL_GUTTER = 32` and `LABEL_GAP = 5` beside the
measurements every chart already shares, and `toPlotRect` drops the gutter parameter
[Enum Metric Chart](../3-11-enum-metric-chart/spec.md) §3.4 gave it — every chart reserved a different width
there, and now none does. `NumericTrendChart`'s `VALUE_AXIS_WIDTH`/`VALUE_LABEL_GAP` and the swimlane's own
gap constant go with it.

Not a tidying. A chart's x scale runs from `plot.left`, so two cards reserving different gutters map the same
moment to different pixels: today a spike on `dense` at day 12 does not sit above `category`'s day-12 mark,
and the offset grows across the window because the plots are different widths too. Each card's own time axis
is correct, so nothing reads as broken — it just isn't comparable, which is what a column of charts over one
window is for.

32px rather than either width in use:

* **Against the Numeric chart's 24px, it repairs a defect.** A value label is right-aligned at
  `plot.left - LABEL_GAP - measureWidth(...)` and never truncated, so one wider than its gutter is drawn off
  the card. At the axis size `1234` measures 20px, `-45.6` 20px and `75000` 25px against the 19px that gutter
  leaves — every four-digit whole number and every negative carrying a decimal clips today. 32px holds all of
  them; only a negative four-digit decimal (`-1234.5`, 30px) still doesn't.
* **Against the Enum chart's 48px, it costs six characters of a lane label.** `low`, `ok`, `high`, `good`,
  `never` and `great` still draw whole; `always` becomes `alwa…` and `excellent` `excel…`. Five characters
  are enough to tell lanes apart when the ramp and the lane's own position already say which is which — the
  argument [Enum Metric Chart](../3-11-enum-metric-chart/spec.md) §3.4 made for capping
  `METRIC_ENUM_VALUE_MAX_LENGTH` rather than widening the gutter to fit it.
* **A Boolean's labels fit either way**: `Yes` measures 15px and `No` 12px, against 27px of room.

The ellipsis stays three dots. Two would save 1.28px, and the narrowest lowercase letter in this face is
`i` at 2.15px — so the saving is smaller than any character it could buy, and buys none anywhere.

Bucketing is untouched: `TIME_RANGE_PRESETS` fixes each preset's `bucketSizeMs` and
`getAggregationForCustomRange` divides its span by a fixed 30, neither of which takes a width. The gutter
changes only how wide a bucket is *drawn* — worst case 30 buckets at `1M`, which on a 320dp phone leaves a
mark 4.8px wide against a `MIN_MARK_WIDTH` of 2.

**`CategorySwimlaneChart`** takes its lanes from `getChartLanes(metric)` in place of `allowedValues`, so the
renderer never asks what type it is drawing. It draws `lane.label` in the shared gutter, truncated against
its width as before, and finds a count's lane by matching `lane.value`. A count matching no lane is
...
axis — is the Enum chart's, and a Boolean Metric differs from an Enum one in nothing the renderer can see
but its lanes. `getLaneColors(2)` already exists and already spans the same endpoints as the 3- and 4-lane
...
1. Open `no numeric` at `1M`. Two cards: `mood` as before, and a new `done` below it — two lanes labelled `Yes`
   ...
   green. Neither word is cut short, and both plots start on the same left edge — `mood`'s has moved right
   against the card since its lane labels lost their wider gutter, and its own labels still read in full.
...
5. Open `mixed metrics` at `1M`. `flag`'s card sits between `insufficient` and `category`, in declaration
   order; `note` still gets none. Ten marks, each filling `Yes` or `No`. Every one of the seven cards now
   starts its plot on the same left edge — sight down the column and the five curves, `flag`'s marks and
   `category`'s all begin together, where the swimlanes previously sat 24px in from the curves.
6. Still on `mixed metrics`, check a Numeric value label that used to be clipped: at `1Y` `yearly` runs into
   three and four digits, and the whole of each label is on the card rather than cut off at its left edge.
7. Switch to `1Y`: `flag` collapses to two columns whose marks are sized by how many Records gave each answer,
   and every Numeric chart still draws its curve, gradient fill, axes, dots and count labels as before.
8. Tap **Add Record** on `mixed metrics`: `flag` still shows Yes/No segments beside its info button, and those
   are the words its chart labels its lanes with.
9. Create an Observation by hand with one Yes/No Metric and one Record: the TRENDS section and its selector
   appear for an Observation that would previously have had neither, with one two-lane card carrying a single
   mark in the answered lane and nothing in the other. Give another a Choice Metric with a value of 9 or more
   characters and confirm it truncates inside the gutter with an ellipsis rather than overrunning the plot.
...
* **Unit — `chartAxis`:** `toPlotRect` reserves `LABEL_GUTTER` whatever chart asks for a rectangle, and still
  collapses an unmeasured box rather than inverting it.
* **Unit — `CategorySwimlaneChart`:** the existing Enum suite passes under the new name, its plot rectangle
  the only thing altered; a Boolean Metric draws two lanes labelled `Yes` over `No` — untruncated, which is
  what says they fit — coloured from `getLaneColors(2)` with `Yes` on the dark entry, its separators and marks
  starting on the same left edge and spanning the same plot an Enum swimlane's do; a bucket where one answer
  was given more often than the other draws the two marks at heights in that ratio, on the same scale as the
  rest of the series; a count matching no lane is neither drawn nor allowed to set that scale; no press ever
  calls `onPointPress`.
* **Regression — `NumericTrendChart`:** its existing suite passes with its plot rectangle moved 8px right and
  nothing else touched — the curve, the hit-testing and the labels all still measured from `plot`.
