...
carries the Numeric cards beside it along as readily as one of their own points would.
...
folding several Records at that resolution stays one bucket, and a swimlane column mixing several values there stays one
column — what a tap separates is time.
...
tap the dot itself, and on a Yes/No or Choice card the column of marks standing over one slice of time, which answers
anywhere down its height. The stretches between belong to the chart, and a tap there leaves the card as it is.
